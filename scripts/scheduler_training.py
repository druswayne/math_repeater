...
from loader import json, router, user_data, user_data_day, user_data_not_start
import os
import time
...
async def scheduler_training(bot: Bot, id_user):
    ...
    message = await bot.send_photo(chat_id=id_user, caption='Что дальше?',
                                         photo=file,
    ...
    if not user_data_not_start[id_user]:
        ...
        await bot.send_message(chat_id=id_user, text=('Ты пропустил сегодня тренировку!\n'
